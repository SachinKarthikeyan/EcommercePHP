<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8" >
		<title>Startup 3.0.5 - Project</title>
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<link rel="icon" href="i/favicon.png" type="image/x-icon">
		<!-- Google Fonts -->
		<link href="https://fonts.googleapis.com/css?family=Rubik:100,200,300,400,600,500,700,800,900|Karla:100,200,300,400,500,600,700,800,900&amp;subset=latin" rel="stylesheet">
		<!-- Bootstrap 4.3.1 CSS -->
		<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
		<!-- Slick 1.8.1 jQuery plugin CSS (Sliders) -->
		<link rel="stylesheet" type="text/css" href="https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.css"/>
		<!-- Fancybox 3 jQuery plugin CSS (Open images and video in popup) -->
		<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/fancyapps/fancybox@3.5.7/dist/jquery.fancybox.min.css" />
		<!-- AOS 2.3.1 jQuery plugin CSS (Animations) -->
		<link href="https://unpkg.com/aos@2.3.1/dist/aos.css" rel="stylesheet">
		<!-- FontAwesome CSS -->
		<link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.8.1/css/all.css" integrity="sha384-50oBUHEmvpQ+1lW4y57PTFmhCaXp0ML5d60M1M7uH2+nqUivzIebhndOJK28anvf" crossorigin="anonymous">
		<!-- Startup 3 CSS (Styles for all blocks) -->
		<link href="css/style.css" rel="stylesheet" />
		<!-- Custom CSS non template -->
		 <link rel="stylesheet" href="css/custom.css">
		<!-- jQuery 3.3.1 -->
		<script src="https://code.jquery.com/jquery-3.3.1.min.js"></script>
		<!-- Fontawesome -->
		<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">

	</head> 
	<body>
		<!-- Navigation Bar -->

		<nav class="navigation_1 bg-light pt-30 pb-30 text-center">
			<div class="container px-xl-0">
				<div class="row justify-content-center align-items-center f-16">
					<div class="mb-20 mb-lg-0 col-lg-3 text-lg-left" data-aos-duration="600" data-aos="fade-down" data-aos-delay="300">
						<a href="#" class="logo link color-main custom-font-a">Clothy <span id="dot">.</span></a>
					</div>
					<div class="col-lg-6" data-aos-duration="600" data-aos="fade-down" data-aos-delay="0">
						<a href="#" class="link color-main mx-15">Home</a>
						<a href="#" class="link color-main mx-15">Shop</a>
						<a href="#" class="link color-main mx-15">Sales</a>
						<a href="#" class="link color-main mx-15">Contact</a>
					</div>
					
				</div>
			</div>
		</nav>

		<!-- Feature 2 -->

		<section class="feature_2 bg-light pt-200 pb-45 text-center">
			<div class="container px-xl-0">
				<div class="row justify-content-center">
					<div class="col-xl-8 col-lg-10">
						<h2 class="large custom-font-a" data-aos-duration="600" data-aos="fade-down" data-aos-delay="0">Clothes are <br>the spirit <br >of fashion <span id="dot">.</span></h2>
						<a href="#" class="btn action-white-bg  md mr-20 link color-main f-16 mt-50 pt-20"><i class="fas fa-arrow-right"></i></a>

					</div>
					<div class="col-xl-10 mt-150">
						<div class="row justify-content-between">
							<div class="mb-50 col-md-4 col-sm-8" data-aos-duration="600" data-aos="fade-down" data-aos-delay="600">
								<div class="color-heading text-adaptive" style="text-align: left;width: 200px;">
									Fashion is a form of self-expression and autonomy at a particular period.
								</div>
							
							</div>
							<div class="mb-50 col-md-4 col-sm-8" data-aos-duration="600" data-aos="fade-down" data-aos-delay="900">
								<div class="color-heading text-adaptive text-start" style="text-align: right;width: 200px;">
									Fashion is a form of self-expression and autonomy at a particular period.
								</div>
							</div>
							
						</div>
					</div>
				</div>
			</div>
		</section>
		<!-- Content 2 -->

		<section class="content_2 bg-dark pt-105 pb-70">
			<div class="container px-xl-0 text-center">
				<div class="row justify-content-center">
					<div class="col-xl-7 col-lg-9 col-md-10" data-aos-duration="600" data-aos="fade-down" data-aos-delay="300">
						<div class="mt-35 f-29 large color-heading custom-font-d">
							Remember to set alerts to follow our upcoming sales and discounts			
						</div>
						<a href="#" class="btn lg action-black f-16 ">ALERTS</a>
					</div>
				</div>
			</div>
		</section>


		<!-- Showcase 2 -->

		<section class="showcase_2 bg-light pt-200 pb-90 text-center">
			<div class="container px-xl-0">
				<div class="row justify-content-center">
					<div class="col-xl-8 col-lg-10">
						<h2 class="small custom-font-m "data-aos-duration="600" data-aos="fade-down" data-aos-delay="0">POPULAR PRODUCTS</h2>
					</div>
				</div>
				<div class="row">
					<div class="col-md-4 col-sm-6" data-aos-duration="600" data-aos="fade-down" data-aos-delay="0">
						<a href="#" class="mt-50 link color-main">
							<img srcset="i/showcase_2_img_1@2x.jpg 2x" src="i/showcase_2_img_1.jpg" class="radius10 img-fluid" alt="" />
							<div class="mt-20 mb-15 f-22 title custom-font-d1">Vaarsity jackets</div>
							<div class="color-heading f-14 semibold text-uppercase sp-20">20 USD</div>
						</a>
					</div>
					<div class="col-md-4 col-sm-6" data-aos-duration="600" data-aos="fade-down" data-aos-delay="300">
						<a href="#" class="mt-50 link color-main">
							<img srcset="i/showcase_2_img_2@2x.jpg 2x" src="i/showcase_2_img_2.jpg" class="radius10 img-fluid" alt="" />
							<div class="mt-20 mb-15 f-22 title custom-font-d1">Vaarsity jackets</div>
							<div class="color-heading f-14 semibold text-uppercase sp-20">20 USD</div>
						</a>
					</div>
					<div class="col-md-4 col-sm-6" data-aos-duration="600" data-aos="fade-down" data-aos-delay="600">
						<a href="#" class="mt-50 link color-main">
							<img srcset="i/showcase_2_img_3@2x.jpg 2x" src="i/showcase_2_img_3.jpg" class="radius10 img-fluid" alt="" />
							<div class="mt-20 mb-15 f-22 title custom-font-d1">Vaarsity jackets</div>
							<div class="color-heading f-14 semibold text-uppercase sp-20">20 USD</div>
						</a>
					</div>
					<div class="col-md-4 col-sm-6" data-aos-duration="600" data-aos="fade-down" data-aos-delay="0">
						<a href="#" class="mt-50 link color-main">
							<img srcset="i/showcase_2_img_4@2x.jpg 2x" src="i/showcase_2_img_4.jpg" class="radius10 img-fluid" alt="" />
							<div class="mt-20 mb-15 f-22 title custom-font-d1">Vaarsity jackets</div>
							<div class="color-heading f-14 semibold text-uppercase sp-20">20 USD</div>
						</a>
					</div>
					<div class="col-md-4 col-sm-6" data-aos-duration="600" data-aos="fade-down" data-aos-delay="300">
						<a href="#" class="mt-50 link color-main">
							<img srcset="i/showcase_2_img_5@2x.jpg 2x" src="i/showcase_2_img_5.jpg" class="radius10 img-fluid" alt="" />
							<div class="mt-20 mb-15 f-22 title custom-font-d1">Vaarsity jackets</div>
							<div class="color-heading f-14 semibold text-uppercase sp-20">20 USD</div>
						</a>
					</div>
					<div class="col-md-4 col-sm-6" data-aos-duration="600" data-aos="fade-down" data-aos-delay="600">
						<a href="#" class="mt-50 link color-main">
							<img srcset="i/showcase_2_img_6@2x.jpg 2x" src="i/showcase_2_img_6.jpg" class="radius10 img-fluid" alt="" />
							<div class="mt-20 mb-15 f-22 title custom-font-d1">Vaarsity jackets</div>
							<div class="color-heading f-14 semibold text-uppercase sp-20">20 USD</div>
						</a>
					</div>
				</div>
			</div>
		</section>

		<!-- Pricing Table 2 -->

		<section class="pricing_table_2 bg-light pt-80 pb-90  text-center">
			<div class="container px-xl-0">
				<div class="col-xl-10 mt-150">
					<div class="row justify-content-between">
						<div class="mb-50 col-md-4 col-sm-8" data-aos-duration="600" data-aos="fade-down" data-aos-delay="600">
							<div class="custom-font-m1">
								WE THINK YOU'LL <span id="title">LIKE THESE</span><span id="dot">   .</span>
							</div>
							<div class="custom-font-d2">
								Fashion is a form of self-expression and autonomy at a particular period.
							</div>
							<div class="row justify-content-between" style="margin-top: 60px;">
								<div class="mb-50 col-md-4 col-sm-8" data-aos-duration="600" data-aos="fade-down" data-aos-delay="600">
									<div class="color-heading text-adaptive">
										<div class="row justify-content-between">
											<div class="mb-50 col-md-4 col-sm-8" data-aos-duration="600" data-aos="fade-down" data-aos-delay="600">
												<div class="color-heading text-adaptive" style="text-align: left;width: 200px;">
													<img srcset="i/testimonial_1_img@2x.jpg 2x" src="i/star.png" class="w-60 h-60 radius_full" alt="" />
												</div>
											
											</div>
											<div class="mb-50 col-md-4 col-sm-8" data-aos-duration="600" data-aos="fade-down" data-aos-delay="900">
												<div class="color-heading text-adaptive text-start" style="text-align: right;width: 200px;">
													<div class="row justify-content-between">
														<div class="mb-50 col-md-4 col-sm-8" data-aos-duration="600" data-aos="fade-down" data-aos-delay="600">
															<div class="color-heading text-adaptive" style="text-align: left;width: 200px;">
															<div class="custom-font-d3">Care Instructions</div>
															<div class="custom-font-d1">Machine was at 30 degree</div>
															</div>
														
														</div>
														<div class="mb-50 col-md-4 col-sm-8" data-aos-duration="600" data-aos="fade-down" data-aos-delay="900">
															<div class="color-heading text-adaptive text-start" style="text-align: right;width: 200px;">
																<img srcset="i/testimonial_1_img@2x.jpg 2x" src="i/star.png" class="w-60 h-60 radius_full" alt="" />
															</div>
														</div>
														
													</div>
												</div>
											</div>
											
										</div>
									</div>
								
								</div>
								<div class="mb-50 col-md-4 col-sm-8" data-aos-duration="600" data-aos="fade-down" data-aos-delay="900">
									<div class="color-heading text-adaptive text-start" style="text-align: left;width: 200px;margin-left: 220px;">
										<div class="custom-font-d3">Fabric Material</div>
										<div class="custom-font-d1">84 % cotton 16 % polyester</div>
									</div>
								</div>
								
							</div>
						</div>
						
						<div class="mb-50 col-md-4 col-sm-8" data-aos-duration="600" data-aos="fade-down" data-aos-delay="900">
							<img src="i/Vector.png" class="img-fluid img-spacing w-2000 h-400" />
						</div>
						
					</div>
				</div>
			</div>
		</section>

		<section class="feature_2 bg-light pb-45 text-center">
			<div class="container px-xl-0">
				<div class="row justify-content-center">
					<div class="col-xl-8 col-lg-10">
						<h2 class="large custom-font-a" data-aos-duration="600" data-aos="fade-down" data-aos-delay="0">FOLLOW US ON</h2>
						<h3 class="custom-font-m" data-aos-duration="600" data-aos="fade-down" data-aos-delay="0">@clothyfashion</h3>
						<div class="row">
							<div class="col-md-4 col-sm-6" data-aos-duration="600" data-aos="fade-down" data-aos-delay="0">
								<a href="#" class="mt-50 link color-main">
									<img srcset="i/showcase_2_img_1@2x.jpg 2x" src="i/showcase_2_img_1.jpg" class="radius10 img-fluid" alt="" />
								</a>
							</div>
							<div class="col-md-4 col-sm-6" data-aos-duration="600" data-aos="fade-down" data-aos-delay="300">
								<a href="#" class="mt-50 link color-main">
									<img srcset="i/showcase_2_img_2@2x.jpg 2x" src="i/showcase_2_img_2.jpg" class="radius10 img-fluid" alt="" />
								</a>
							</div>
							<div class="col-md-4 col-sm-6" data-aos-duration="600" data-aos="fade-down" data-aos-delay="600">
								<a href="#" class="mt-50 link color-main">
									<img srcset="i/showcase_2_img_3@2x.jpg 2x" src="i/showcase_2_img_3.jpg" class="radius10 img-fluid" alt="" />
								</a>
							</div>
							
						</div>
					</div>
				</div>
			</div>
		</section>

		
		<!-- Footer 1 -->

		<footer class="footer_1 bg-light pt-75 pb-65 text-center">
			<div class="container px-xl-0">
				<div class="row justify-content-between align-items-center lh-40 links">
					<div class="col-lg-4 col-sm-6 text-sm-right text-lg-left order-1 order-lg-0" data-aos-duration="600" data-aos="fade-down" data-aos-delay="300">
						<a href="#" class="mr-15 link color-main">About</a>
						<a href="#" class="mx-15 link color-main">Policy</a>
						<a href="#" class="mx-15 link color-main">Terms</a>
					</div>
					<div class="mb-10 mb-lg-0 col-lg-auto order-0" data-aos-duration="600" data-aos="fade-down" data-aos-delay="0">
						<a href="#" class="logo link color-main custom-font-a">Clothy <span id="dot">.</span></a>
					</div>
					<div class="col-lg-4 col-sm-6 text-sm-left text-lg-right order-2 order-lg-0" data-aos-duration="600" data-aos="fade-down" data-aos-delay="300">
						<a href="#" class="mr-15 link color-main">Contacts</a>
						<a href="#" class="mx-15 link color-main"><i class="fab fa-twitter"></i></a>
						<a href="#" class="mx-15 link color-main"><i class="fab fa-facebook-square"></i></a>
						<a href="#" class="ml-15 link color-main"><i class="fab fa-google-plus-g"></i></a>
					</div>
				</div>
				<div class="row justify-content-center">
					<div class="mt-10 col-xl-4 col-lg-5 col-md-6 col-sm-8" data-aos-duration="600" data-aos="fade-down" data-aos-delay="0">
						<div class="color-heading text-adaptive">
							Be sure to take a look at our <a href="#" class="link color-heading">Terms of Use</a> <br />
							and <a href="#" class="link color-heading">Privacy Policy</a>
						</div>
					</div>
				</div>
			</div>
		</footer>

		<!-- Bootstrap 4.3.1 JS -->
		<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.bundle.min.js"></script>
		<!-- Fancybox 3 jQuery plugin JS (Open images and video in popup) -->
		<script src="https://cdn.jsdelivr.net/gh/fancyapps/fancybox@3.5.7/dist/jquery.fancybox.min.js"></script>
		<!-- Google maps JS API -->
		<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?v=3&key=AIzaSyDP6Ex5S03nvKZJZSvGXsEAi3X_tFkua4U"></script>
		<!-- Slick 1.8.1 jQuery plugin JS (Sliders) -->
		<script type="text/javascript" src="https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js"></script>
		<!-- AOS 2.3.1 jQuery plugin JS (Animations) -->
		<script src="https://unpkg.com/aos@2.3.1/dist/aos.js"></script>
		<!-- Maskedinput jQuery plugin JS (Masks for input fields) -->
		<script src="js/jquery.maskedinput.min.js"></script>
		<!-- Startup 3 JS (Custom js for all blocks) -->
		<script src="js/script.js"></script>
	</body>
</html>